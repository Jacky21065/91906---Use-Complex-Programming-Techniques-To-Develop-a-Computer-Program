...
Program Version: maths_car_race
Author: Jacky
Date: Thursday, 24th April 2025
Purpose/Description: Concept Program 1.
Langauge Used: Python 3.10
Files Required: Images
Known Issues: Car movement needs to be realigned to the 2 lanes on the lanes of the background image.
"""
...
# Function to create a with text to go with the rectangle
...
def gameloop():
    ...
    bg_speed = 6  
    # Initial X position of car (centered)
    car_x = ((DISPLAY_WIDTH / 2) - (CAR_WIDTH / 2))
    # Constant Y position of car (bottom of screen)
    car_y = (DISPLAY_HEIGHT - CAR_HEIGHT)
    # Horizontal movement tracker
    car_x_change = 0  
    ...
    # Set random X positions for the obstacle car to appear in
    car_startx = random.randrange(road_start_x, road_end_x - CAR_WIDTH)
    # Start y-position for obstacle
    car_starty = -600
    # Obstacle width (constant with the player's car)
    carw = 50
    # Obstacle height (constant with the player's car)
    carh = 100
    ...
    while not gameExit:  
        ...
        for event in pygame.event.get(): 
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # Then shift the player car horizontally by -5 x (left)
                if event.key == pygame.K_LEFT:  
                    car_x_change = -5
                # Otherwise, if the right arrow key is being pressed
                # Then shift the player car horizontally by +5 x (right)
        ...
        # Creae Rect objects for obstacle car
        obstacle_rect = pygame.Rect(car_startx, car_starty, carw, carh)
        ...
        car(car_x, car_y)
        # Draw obstacle car
        draw_cars(car_startx, car_starty, car2Img)
        # Display score
        score(count)
...
introduction_screen()
# Start the main game loop, which encapsulate the code into a
