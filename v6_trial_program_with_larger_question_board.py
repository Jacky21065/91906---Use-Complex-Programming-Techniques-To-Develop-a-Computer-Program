...
Version Name: v6_trial_program_with_larger_question_board
Author: Jacky
Date: Tuesday, 1st July 2025
Purpose/Description: Program to trial and test. 
Language Used: Python 3.10
Files Required: Images, Music
Known Issues: 
Fixed Issues: Integral signs show up correctly
"""
...
def generate_calculus_question(mode=None):
    ...
    if mode == "diff":
        ...
        answer = str(deriv).replace('**', '^').replace('*', '')
        return question, answer

    elif mode == "int":
        integral = sp.integrate(poly, x)
        question = f"∫ {coeff}x^{power} dx"
        answer = str(integral).replace('**', '^').replace('*', '')
        return question, answer
...
def generate_question():
    ...
    if selected_difficulty == "Beginner":
        ...
        # Easy mode includes Beginner difficulty logic
        # as well as fractions and exponents
        ...
        if mode == "basic":
            ...
            b = random.randint(1, 10)
            a = b * random.randint(1, 10)  # ensures a is divisible by b
            question = f"{a}/{b}"
            return question, a // b
        ...
        # Generate linear equations with integer solution for x
        x = sp.Symbol('x')
        a = random.randint(1, 10)
        sol = random.randint(-10, 10)  # integer solution for x
        b = random.randint(-10, 10)
        c = a * sol + b  # ensures a * x + b = c has integer x
        eq = sp.Eq(a * x + b, c)
        question = f"Solve: {a}x + {b} = {c}"
        return question, sol

    elif selected_difficulty == "Hard":
        return generate_calculus_question(mode=random.choice(["diff", "int"]))
...
(1=Yes, 0=No)"""
            answer = 1 if sp.isprime(num) else 0
            return question, answer
...
def gameloop():
    ...
    while not gameExit:
        ...
        message_display(current_question, 70, road_start_x + question_box_width // 2, 50, WHITE)
...
def show_leaderboard_screen():
    ...
    # Title
    tk.Label(leaderboard_window, text="Leaderboard - Top 10", font=("SegoeUI", 18, "bold"), bg="white").pack(pady=10)
...
tk.Button(btn_frame, text="Leaderboard", font=("SegoeUI", 14), width=16, command=lambda: show_leaderboard_screen()).pack(pady=10)
...
def show_end_screen():
    ...
    tk.Label(end_root, text=f"Average Answer Time: {avg_answer_time:.2f} seconds", font=("SegoeUI", 16), bg="skyblue").pack(pady=10)
    ...
    # Mistakes button — show mistakes log window
    def show_mistakes_screen():
        mistakes = load_mistakes()

        if not mistakes:
            messagebox.showinfo("Mistakes", "No mistakes logged yet.")
            return

        mistakes_window = tk.Toplevel()
        mistakes_window.title("Mistakes")
        mistakes_window.geometry("900x600")
        mistakes_window.configure(bg="cornflowerblue")
        mistakes_window.resizable(False, False)

        tk.Label(mistakes_window, text="Mistakes Log", font=("SegoeUI", 18, "bold"), bg="white").pack(pady=10)

        text_frame = tk.Frame(mistakes_window, bg="white")
        text_frame.pack(padx=20, pady=10, fill="both", expand=True)

        header = f"{'Username':<15} {'Difficulty':<10} {'Question':<30} {'Correct Answer':<20} {'Player Answer':<20}"
        tk.Label(text_frame, text=header, font=("Courier New", 12, "bold"), bg="white").pack(anchor='w')

        for entry in mistakes[-50:]:
            username = entry.get("Username", "N/A")
            diff = entry.get("Difficulty", "N/A")
            question = entry.get("Question", "N/A")
            correct_ans = entry.get("Correct_Answer", "N/A")
            player_ans = entry.get("Player_Answer", "N/A")

            line = f"{username:<15} {diff:<10} {question:<30} {correct_ans:<20} {player_ans:<20}"
            tk.Label(text_frame, text=line, font=("Courier New", 11), bg="white").pack(anchor='w')
    ...
    tk.Button(end_root, text="Mistakes", font=("SegoeUI", 16), width=15, command=show_mistakes_screen).pack(pady=10)
